...
export const NavBar = () => {
    ...
    return (
        <>
            <nav>
                <a className="logoChillIt" href="/"><img src={logo} alt=''/></a>
                <ul>
                    ...
                    <li className={`normalLi ${currentPage === 'Why Chill it?' ? 'selected' : ''}`} onClick={() => setCurrentPage('Why Chill it?')}><Link to='/' onClick={() => {
    setTimeout(() => {
        window.scrollBy(0, 0);
        ScrollTrigger.refresh();
    }, 100); // Ajusta el retraso según sea necesario
}} style={{ color: 'inherit', textDecoration: 'inherit'}}>Why Chill it?</Link></li>
